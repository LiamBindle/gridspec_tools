...
char tagname[] = "$Name: quebec_prerelease3 $";
  ...
  */
  ...
  for(m=0; m<file_in->nt; m++) {
	...
	get_test_input_data(test_case, test_param, ntiles_in, scalar_in, grid_in, bound_T, opcode);
        ...
        get_input_data(ntiles_in, scalar_in, grid_in, bound_T, l, level);
    ...
    for(l=0; l<nvector; l++) {
      ...
      get_input_data(ntiles_in, u_in, grid_in, bound_T, l, level);
      get_input_data(ntiles_in, v_in, grid_in, bound_T, l, level);
